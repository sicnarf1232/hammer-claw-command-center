...
- Brain chat attachments (dev-feedback #6): paperclip, drag-drop, and paste upload in the Brain composer (images, PDFs, txt/md/csv/log/json; max 4 per message, 8 MB each, enforced client- and server-side). Files store as private Vercel Blobs via `POST /api/brain/upload`; `/api/thread-chat` fetches the bytes and sends real Anthropic image/document blocks (text files inline inside `<untrusted_content>`, trust boundary extended to uploaded files). Only the latest user turn re-ships attachment bytes; older turns keep a `[attached: name]` marker (server-side trim in `lib/brainAttachments.ts`, 20 new tests). History chips are metadata only, never bytes in localStorage. Works on both smart and fast models.

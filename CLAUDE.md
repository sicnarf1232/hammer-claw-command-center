...
| Auth | Single-user. Vercel password protection OR a shared-secret middleware. | No Clerk/Auth0 unless asked. |
| Email | Microsoft Power Automate flows -> app webhooks | App never talks to Graph API directly. See docs/03. |
...
5. **Workstream identity is sacred.** The vault is split into workstreams (merit, nextech, sloan, personal, shared). Output that goes anywhere with an identity (an email draft, a filed note) must use the right workstream's folder, email, and brand. See docs/02 section "Workstreams." When in doubt, ask.
